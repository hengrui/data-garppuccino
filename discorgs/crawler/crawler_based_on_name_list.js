	...
	async.series({
		'init': function(callback){
			...
			try{
				...
				console.log('-------fail on initial stage-------');
				return ;
			}
		},
				...
				function (callback){
					...
					setTimeout(function(){
						...
						try{
							db.search(input_name, params, function(err, data){
								if (data == undefined){
									//throw new Error('----Error on finding page: ' + current_page + '-----------');
								}
								else
					...
					setTimeout(function(){
						...
						try{
							db.release(release_array[item], function(err, data){
								//data = null;
								if (data == undefined){
									//throw new Error('-------Error on finding release id: ' + release_array[item] + '------');
			...
			var artist_timeout = setInterval(function(){		
				...
				try {
					...
					db.artist(artist_array[item], function(err, data){
						if (data == undefined){
							//throw new Error('-----Undefined artist data on airtist id = ' + artist_array[item] + '-------');
						}
						else if (data.message == undefined){
							if (data.id == undefined){
								//throw new Error('------Undefined artist id for ' + artist_array[item] + '--------');
							}
