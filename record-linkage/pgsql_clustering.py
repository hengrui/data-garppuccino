...
def upload_blocks():
    c.execute("DROP TABLE IF EXISTS %s.blocking_map" % SCHEMA)
    c.execute("CREATE TABLE {0}.blocking_map "
              "(block_key VARCHAR(200), merged_id BIGINT);".format(SCHEMA))
    c.execute("CREATE INDEX {0}_blocking_map_key_idx ON {0}.blocking_map (block_key);".format(SCHEMA))
    con.commit();

    with open('blocks_', 'r') as f:
        #csv_file.close()
        print 'transfering blocking map'
        c.copy_expert("COPY {0}.blocking_map FROM STDIN CSV;".format(SCHEMA), f)
        f.close()
        con.commit()
...
def prepareBlocks():
    ...
    c.execute("DROP TABLE IF EXISTS {0}.plural_key".format(SCHEMA))
    c.execute("DROP TABLE IF EXISTS {0}.plural_block".format(SCHEMA))
    c.execute("DROP TABLE IF EXISTS {0}.covered_blocks".format(SCHEMA))
    c.execute("DROP TABLE IF EXISTS {0}.smaller_coverage".format(SCHEMA))
    ...
    c.execute("CREATE TABLE {0}.plural_key "
              "(block_key VARCHAR(200), "
              " block_id SERIAL PRIMARY KEY)".format(SCHEMA))

    c.execute("INSERT INTO {0}.plural_key (block_key) "
              "SELECT block_key FROM {0}.blocking_map "
              "GROUP BY block_key HAVING COUNT(*) > 1".format(SCHEMA))

    logging.info("creating block_key index")
    c.execute("CREATE UNIQUE INDEX {0}_block_key_idx ON {0}.plural_key (block_key)".format(SCHEMA))

    logging.info("calculating plural_block")
    c.execute("CREATE TABLE {0}.plural_block "
              "AS (SELECT DISTINCT block_id, merged_id "
              " FROM {0}.blocking_map INNER JOIN {0}.plural_key "
              " USING (block_key))".format(SCHEMA))

    logging.info("adding index and sorting index")
    c.execute("CREATE INDEX {0}_plural_block_merged_id_idx ON {0}.plural_block (merged_id)".format(SCHEMA))
    c.execute("CREATE UNIQUE INDEX {0}_plural_block_block_id_merged_id_uniq "
              " ON {0}.plural_block (block_id, merged_id)".format(SCHEMA))
    ...
    # ids, so we'll also increase the maximum strping length to try to
    # avoid this.
    # c.execute("SET group_concat_max_len = 2048")

    logging.info("creating covered_blocks")
    c.execute("CREATE TABLE {0}.covered_blocks "
              "AS (SELECT merged_id, "
              " string_agg(CAST(block_id AS TEXT), ',' ORDER BY block_id) "
              "   AS sorted_ids "
              " FROM {0}.plural_block "
              " GROUP BY merged_id)".format(SCHEMA))

    c.execute("CREATE UNIQUE INDEX {0}_covered_blocks_merged_id_idx "
              "ON {0}.covered_blocks (merged_id)".format(SCHEMA))
    con.commit()
    ...
    c.execute("CREATE TABLE {0}.smaller_coverage "
              "AS (SELECT merged_id, block_id, "
              " TRIM(',' FROM split_part(sorted_ids, CAST(block_id AS TEXT), 1)) "
              "      AS smaller_ids "
              " FROM {0}.plural_block INNER JOIN {0}.covered_blocks "
              " USING (merged_id))".format(SCHEMA))
    con.commit()
...
def clustering():
    ...
    c4.execute("SELECT t.*, "
               ...
               "FROM {0}.smaller_coverage "
               "INNER JOIN {1} as t "
               "USING (merged_id) "
               "ORDER BY (block_id)".format(SCHEMA, TABLE))
    ...
    c.execute("CREATE TABLE {0}.entity_map "
              ...
              " cluster_score FLOAT, PRIMARY KEY(merged_id))".format(SCHEMA))
    ...
    c.copy_expert("COPY {0}.entity_map FROM STDIN CSV".format(SCHEMA), f)
    f.close()
    ...
    c.execute("CREATE INDEX {0}_head_index ON {0}.entity_map (canon_id)".format(SCHEMA))
    con.commit()
